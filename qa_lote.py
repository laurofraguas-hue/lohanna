...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        print(f"{'PAINEL':44s}{'ERR':>4s}{'NET':>4s}{'PROIB':>6s}{'CARD':>5s}{'INS':>4s}{'MAPA':>6s}{'ACORD':>6s}{'TAB':>5s}{'TOP':>4s}{'ALVO':>5s}{'CNV':>4s}{'MOB':>5s}{'SEL':>5s}{'RESP':>6s}")
        print("-"*130)
        for p in sorted(glob.glob("/home/user/lohanna/paineis/*.html")):
            ...
            print(f"{nome[:44]:44s}{len(r['erros']):>4d}{len(r['req']):>4d}{prob:>6d}"
                  ...
                  f"{r['mapaMob']:>5d}{'ok' if r['trocou'] else 'FALHA':>5s}{'ok' if resp_ok else 'FALHA':>6s}"
